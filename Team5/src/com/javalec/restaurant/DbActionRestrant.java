...
//		String query = "select code, name, location from restaurant";
		...
		try {
			...
			while(rs.next()) {
				...
				BeanRestrant beanRestrant = new BeanRestrant(wkNum, wkName, wkLocation);
	...
	// 식당 클릭
//	public ArrayList<BeanRestrant> Allergy1List(String wkName) {
//		
//		ArrayList<BeanRestrant> beanList = new ArrayList<BeanRestrant>();
//		
...
//	}
	...
	public BeanRestrant restaurantInfo(String wkNumS) {
		
		BeanRestrant beanRestrant = null;
		
		int wkNum = Integer.parseInt(wkNumS);
	...
	public String restaurantStar(String wkNumS){
		
		int wkNum = Integer.parseInt(wkNumS);
		String wkStar = "";
		String query = "select sum(length(star)/3)/count(star) from review where r_code='" + wkNum + "'";	// 해당 식당의 코드로
...
//		String query = "select rt.name, sum(length(rw.star)/3)/count(rw.star), rt.location from restaurant as rt, review as rw where rt.code = rw.r_code group by rw.r_code";
